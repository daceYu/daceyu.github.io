...
PASSPORT.createHint = function(o, inner) {
    if (arguments.length > 1) {
        var $html = '<'+ o.tag="" +="" '="" class="' + o.hint + ' ' + o.state + '"><' +="" inner="" '="">' + o.text + '</'>';
    } else {
        // 返回提示
        var $html = $('<'+ o.tag="" +="" '="" class="' + o.hint + ' ' + o.state + '">');
        $html.html(o.text);
    }
    return $html;
};
...
(function($) {
    ...
    $.fn.passportPopup = function(options) {
        ...
        function countdown(time, node) {
            ...
            // 1. time是设定的倒计时时间;
            // 2. node是自定义显示倒计时的位置;
            if ((typeof time != 'number') || time <= 1)="" throw="" new="" error('参数类型错误或小于等于1');="" var="" _time="Math.ceil(time" -="" 1);="" _popup="$("#"" +="" _popupid);="" 显示倒计时="" _timernum();="" 清除倒计时="" window.cleartimeout(this._t);="" 倒计时开始="" this._t="window.setTimeout(function()" {="" if="" (_time=""> 1) {
                    // 显示倒计时
                    _timerNum();
...
//	passportReghtml +='</div></div>';
	...
	passportReghtml +='<div class="passport-sms getVoice" style="display: none;">未收到短信？使用<a href="javascript:;" class="js-getvoice" jktag="0001|0.1|91044" target="_blank" rel="external">语音动态码</a></div>';
...
$(function() {
    SIGN.init();
})</div></div></div></div></div></div></=></div></div></'+></'+></=i[1]},equalto:function(e,i,s){var></=s[1]},min:function(t,e,i){return></13||t.length></0&&(this.pendingrequest=0),delete></"+this.settings.wrapper+"></"+this.settings.errorelement+"></arguments.length;t++)if(void>